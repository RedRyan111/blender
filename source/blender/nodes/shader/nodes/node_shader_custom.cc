...
namespace blender::nodes::node_shader_custom_cc {

static void sh_node_custom_declare(NodeDeclarationBuilder &b)
{
  b.is_function_node();
...
static void node_shader_buts_custom(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  uiItemR(layout, ptr, "clamp_type", UI_ITEM_R_SPLIT_EMPTY_NAME, "", ICON_NONE);
}

static void node_shader_init_custom(bNodeTree * /*ntree*/, bNode *node)
{
  node->custom1 = NODE_CLAMP_MINMAX; /* clamp type */
}

static int gpu_shader_custom(GPUMaterial *mat,
                            bNode *node,
...
static void sh_node_custom_build_multi_function(NodeMultiFunctionBuilder &builder)
{
...
void register_node_type_sh_custom() //add to node_shader_register 
{
  namespace file_ns = blender::nodes::node_shader_custom_cc;

  static blender::bke::bNodeType ntype;

  //all sh_node_... was ddeclared in this file
  sh_fn_node_type_base(&ntype, SH_NODE_CUSTOM, "Custom", NODE_CLASS_CONVERTER);
  ntype.declare = file_ns::sh_node_custom_declare;
  ntype.draw_buttons = file_ns::node_shader_buts_custom;
  ntype.initfunc = file_ns::node_shader_init_custom;
  ntype.gpu_fn = file_ns::gpu_shader_custom;
  ntype.build_multi_function = file_ns::sh_node_custom_build_multi_function;
